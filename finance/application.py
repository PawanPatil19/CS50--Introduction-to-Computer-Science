...
def login():
    ...
    if request.method == "POST":
        ...
        elif not request.form.get("captcha"):
            return apology("must provide captcha", 403)
        ...
        if len(rows) != 1 or not check_password_hash(rows[0]["hash"], request.form.get("password")) or captcha!="RBSKW":
             return apology("invalid username and/or password ans/or captcha", 403)
...
def add():
    ...
    if request.method == "POST":
        ...
        if not money:
            return apology("Enter the amount of money")

        # Calculate total value of the transaction

        cash = db.execute("SELECT cash FROM users WHERE id = :user",
                          user=session["user_id"])[0]['cash']
        cash_after = cash + money
